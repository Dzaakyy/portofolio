    ...
    >
        ...
        >
            ...
            <div
                ...
                :style="{
                    backgroundImage:
                        'radial-gradient(rgba(235, 235, 235, 0) 65%, var(--overlay-blur-color) 100%)',
                }"
            />
            <div
                class="absolute inset-0 m-auto z-[3] pointer-events-none"
                :style="{
                    WebkitMaskImage:
                        'radial-gradient(rgba(235, 235, 235, 0) 70%, var(--overlay-blur-color) 90%)',
                    maskImage:
                        'radial-gradient(rgba(235, 235, 235, 0) 70%, var(--overlay-blur-color) 90%)',
            ...
            />
                   ...
                   <div
         class="absolute left-0 top-0 bottom-0 w-[120px] z-[5] pointer-events-none"
         :style="{
           background:
             'linear-gradient(to right, var(--overlay-blur-color, #060010), transparent)',
         }"
       />

       <div
         class="absolute right-0 top-0 bottom-0 w-[120px] z-[5] pointer-events-none"
         :style="{
           background:
             'linear-gradient(to left, var(--overlay-blur-color, #060010), transparent)',
         }"
       />
...
const onDragMove = (e: MouseEvent | TouchEvent) => {
    if (focusedElement.value || !isDragging.value || !startPosition.value)
        return;
...
const closeEnlargedImage = () => {
    ...
    const cleanup = () => {
        ...
        requestAnimationFrame(() => {
            ...
            rootRef.value?.removeAttribute("data-enlarging");
            scrimRef.value?.classList.add("opacity-0", "pointer-events-none");
            scrimRef.value?.classList.remove(
                "opacity-100",
                "pointer-events-auto",
            );
            requestAnimationFrame(() => {
